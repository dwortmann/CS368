 ...
 */
	...
	if(this != &rhs)
	{
	   freeList(head); //clean up whatever we have now
	   head = copyList(rhs.head); //copy over the list
	}
    }

    //Free up a list of linked nodes starting at the head node
    ...
    //Copy an existing list into a new one (hard copy)
